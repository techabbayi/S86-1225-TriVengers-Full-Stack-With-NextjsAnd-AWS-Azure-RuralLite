...
export default function LessonDetailPage({ params }) {
  ...
  const getQuizCompletionForLesson = () => {
    ...
    const attempts = quizHistory.filter(
      (h) => h.quizId === (subjectQuiz._id || subjectQuiz.id)
    );
  ...
  const fetchProgress = async () => {
    try {
      ...
      if (data.success) {
        const lessonProgress = data.data.find(
          (p) => p.lessonId === unwrappedParams.id
        );
  ...
  const cleanTextForSpeech = (markdownText) => {
    ...
    if (cleaned.length > 15000) {
      cleaned =
        ...
        "... Content truncated for audio playback.";
    }
  ...
  const startSpeaking = () => {
    ...
    try {
      ...
      newUtterance.onerror = (event) => {
        ...
        alert(
          `Audio playback error: ${event.error || "Unknown error"}. Please try again.`
        );
      };
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-600">
                          Audio narration of lesson content
                        </p>
                      </div>
                    </div>
                  ...
                  <div className="text-center sm:text-left">
                    ...
                    {quizCompletion && (
                      <div className="mt-2 inline-flex items-center gap-2 bg-green-100 text-green-700 px-4 py-2 rounded-lg font-semibold">
                        ...
                        Passed · Score: {quizCompletion.score}/
                        {quizCompletion.totalQuestions}
                      </div>
                    )}
                  </div>
                  <button
                    onClick={async () => {
                      ...
                      try {
                        ...
                        if (data.success && data.data) {
                          const subjectQuiz = data.data.find(
                            (q) => q.subject === lesson.subject
                          );
                          if (subjectQuiz) {
                            // Navigate directly to quiz detail page
                            router.push(
                              `/quizzes/${subjectQuiz._id || subjectQuiz.id}`
                            );
                          } else {
                            toast.error(
                              "No quiz available for this subject yet"
                            );
                          }
                        }
                      } catch (err) {
