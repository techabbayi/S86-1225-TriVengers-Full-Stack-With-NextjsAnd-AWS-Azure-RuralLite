  ...
  const handleFileChange = (e) => {
    ...
    if (selectedFile) {
      ...
      if (selectedFile.size > maxFileSize) {
        setUploadStatus(
          `❌ File size exceeds limit (${maxFileSize / (1024 * 1024)}MB)`
        );
        setFile(null);
        return;
      }
  ...
  const uploadFile = async () => {
    ...
    try {
      ...
      if (!urlResponse.ok) {
        ...
        } else if (urlResponse.status === 413) {
      ...
      if (!metadataResponse.ok) {
        if (metadataResponse.status === 401) {
          throw new Error("Please log in to save file information");
        } else {
          throw new Error(
            "File uploaded but failed to save details. Please contact support."
          );
        }
      }
